...
namespace clipper
{
  ...
  /*! This is the import/export class which can be linked to a CCP4 map
    ...
    data structure. */
  class CLIPPER_IMEX CCP4MAPfile
  {
   public:
    ...
    template<class T> ASUerror import_xmap_check_asu( Xmap<T>& xmap, T missing ) const;

  private:
	//// Exists only to force the compiler to instantiate templates
	//  void _donotcall() {
	//	  Xmap<ftype32> xmap32;
	//	  Xmap<ftype64> xmap64;
	//	  NXmap<ftype32> nxmap32;
	//	  NXmap<ftype64> nxmap64;

	//	  import_xmap(xmap32);
	//	  export_xmap(xmap32);
	//	  import_xmap(xmap64);
	//	  export_xmap(xmap64);
	//	  import_nxmap(nxmap32);
	//	  export_nxmap(nxmap32);
	//	  import_nxmap(nxmap64);
	//}
  ...
  protected:
    ...
    MAPmode mode;     //!< mode
    ASUerror asuerr;  //!< ASU error flag
