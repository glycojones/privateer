...
#define CCP4_VERSION_NO "7.0"
#define CCP4_PATCH_LEVEL "7.0.0"
...
CCP4_IMEX typedef int (*CCP4INTFUNCPTR)(int, const char *);
 ...
 */
CCP4_IMEX char *ccp4_prog_vers(const char *progvers);

/** Query ccp4 version.
 * @return CCP4 version string.
 */
CCP4_IMEX char *ccp4_vers_no(void);
 ...
 * @return Date string
 */
CCP4_IMEX char *ccp4RCSDate(const char *rcs_string);

/** Set or print program time information
 * @param init
 */
CCP4_IMEX void ccp4ProgramTime(int init);
 ...
 */
CCP4_IMEX int ccp4VerbosityLevel(int level);
 ...
 */
CCP4_IMEX int ccp4Callback(CCP4INTFUNCPTR mycallback, char *mode, int ierr,
                 const char *message);
 ...
 */
CCP4_IMEX int ccp4InvokeCallback(int ierr, const char *message);
 ...
 */
CCP4_IMEX int ccp4_licence_exists(const char *name);
 ...
 */
CCP4_IMEX int html_log_output(int ihtml_in);
