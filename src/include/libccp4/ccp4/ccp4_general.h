...
CCP4_IMEX void ccp4f_mem_tidy(void);

CCP4_IMEX int ccperror(int ierr, const char *message);

CCP4_IMEX int ccperror_noexit(int ierr, const char *message);

CCP4_IMEX int ccp4printf(int level, char *format, ...);

CCP4_IMEX int ccp4fyp(int argc, char **argv);

CCP4_IMEX int ccp4fyp_cleanup(int ienv, char **envname, char **envtype, char **envext,
		    char *logical_name, char *file_name, char *file_type,
		    char *file_ext, char *env_file, char *def_file,
		    char *dir, CCP4PARSERARRAY *parser);

CCP4_IMEX int ccp4setenv(char *logical_name, char* value, char **envname,
	       char **envtype, char **envext, int *ienv, int no_overwrt);

CCP4_IMEX int ccp4setenv_cleanup(char *file_ext, char *file_root, char *file_path,
		       char *file_name);

CCP4_IMEX int ccpexists(char *filename);

CCP4_IMEX int ccpputenv(char *logical_name, char *file_name);

CCP4_IMEX void ccp4_banner(void);
