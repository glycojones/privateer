...
pybind11::dict privateer::pyanalysis::GlycosylationInteractions::get_chpibonds_for_specific_glycan(int glycanIndex)
{
    ...
    if(output.size() > 0)
    {
        ...
        for(int i = 0; i < output.size(); i++)
        {
            ...
            auto targetSugar = pybind11::dict("targetSugarChainID"_a=output[i].get_sugar_chainID(), "target_sugar_type"_a=target_sugar_type, "target_sugar_seqnum"_a=target_sugar_seqnum, "target_sugar_pdb_id"_a=target_sugar_pdb_id, "target_sugar_face"_a=output[i].get_sugar_face(), "target_sugar_c_atom"_a=target_sugar_c_atom);
