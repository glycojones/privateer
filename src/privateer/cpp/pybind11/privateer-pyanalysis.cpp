...
pybind11::dict privateer::pyanalysis::GlycosylationInteractions::get_chpibonds_for_specific_glycan(int glycanIndex)
{
    ...
    if(output.size() > 0)
    {
        ...
        for(int i = 0; i < output.size(); i++)
        {
            ...
            std::string target_sugar_h_atom = output[i].get_xh_pair().second.id().trim();
            std::string glycan_type = inputGlycan.get_type();
            auto targetSugar = pybind11::dict("targetSugarChainID"_a=output[i].get_sugar_chainID(), "target_sugar_type"_a=target_sugar_type, "target_sugar_seqnum"_a=target_sugar_seqnum, "target_sugar_pdb_id"_a=target_sugar_pdb_id, "target_sugar_face"_a=output[i].get_sugar_face(), "target_sugar_c_atom"_a=target_sugar_c_atom, "target_sugar_h_atom"_a=target_sugar_h_atom);
            ...
            auto entry = pybind11::dict("sugarIndex"_a=output[i].get_sugar_index(), "glycanSize"_a=output[i].get_glycan_size(), "glycan_type"_a=glycan_type, "co_distance"_a=output[i].get_distance_xo(), "hudson_theta_angle"_a=output[i].get_angle_theta_h(), "hudson_cp_distance"_a=output[i].get_distance_xp(), "plevin_theta_angle"_a=output[i].get_angle_theta_p(), "plevin_phi_angle"_a=output[i].get_angle_phi(), "targetSugar"_a=targetSugar, "stackedResidue"_a=stackedResidue);
