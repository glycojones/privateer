...
std::string privateer::pyanalysis::GlycosylationInteractions::convert_three_letter_code_to_single_letter(std::string three_letter_code)
{
    std::unordered_map<std::string, std::string> code_conversion =
        {
            {"ALA", "A"}, {"ARG", "R"}, {"ASN", "N"}, {"ASP", "D"}, {"CYS", "C"}, {"GLN", "Q"}, {"GLU", "E"}, {"GLY", "G"}, {"HIS", "H"}, {"ILE", "I"}, {"LEU", "L"}, {"LYS", "K"}, {"MET", "M"}, {"PHE", "F"}, {"PRO", "P"}, {"SER", "S"}, {"THR", "T"}, {"TRP", "W"}, {"TYR", "Y"}, {"VAL", "V"}, {"HYP", "P"}, {"LYZ", "K"}, {"SEP", "S"}, {"DA", "A"}, {"DC", "C"}, {"DG", "G"}, {"DT", "T"}};
...
void privateer::pyanalysis::GlycosylationComposition::read_from_file(std::string path_to_model_file, std::string expression_system, bool disable_torsions, bool debug_output)
{
    ...
    for (int index = 0; index < ligandList.size(); index++)
    {
        ...
        {
            if (sugarList[index][natom].coord_orth().x() > maxX)
                maxX = sugarList[index][natom].coord_orth().x(); // calculation of the sugar centre
            if (sugarList[index][natom].coord_orth().y() > maxY)
                maxY = sugarList[index][natom].coord_orth().y();
            if (sugarList[index][natom].coord_orth().z() > maxZ)
                maxZ = sugarList[index][natom].coord_orth().z();
            if (sugarList[index][natom].coord_orth().x() < minX)
                minX = sugarList[index][natom].coord_orth().x();
            if (sugarList[index][natom].coord_orth().y() < minY)
                minY = sugarList[index][natom].coord_orth().y();
            if (sugarList[index][natom].coord_orth().z() < minZ)
                minZ = sugarList[index][natom].coord_orth().z();
        }
...
pybind11::list privateer::pyanalysis::GlycosylationComposition::get_torsions_per_linkage_summary()
{
    ...
    for (int i = 0; i < totalGlycans; i++)
    {
        ...
        pybind11::list current_glycan_torsions = currentGlycan.get_torsions_per_linkage_summary();

        auto current_glycan_torsion_info = pybind11::dict("glycanIndex"_a = i, "WURCS"_a = currentGlycan.get_wurcs_notation(), "torsions"_a = current_glycan_torsions);
        output.append(current_glycan_torsion_info);
    }
...
pybind11::list privateer::pyanalysis::GlycosylationComposition::get_torsions_zscore_summary_with_pdb(OfflineTorsionsZScoreDatabase &importedDatabase, std::string input_pdb_code)
{
    if (!torsions.empty())
        return torsions;
    else
    {
        ...
        {
            privateer::pyanalysis::GlycanStructure currentGlycan = get_glycan(i);
            pybind11::list current_glycan_zscore_torsions = currentGlycan.get_torsions_zscore_summary_with_pdb(importedDatabase, input_pdb_code);
...
pybind11::list privateer::pyanalysis::GlycosylationComposition_memsafe::get_torsions_per_linkage_summary()
{
    // std::cout << "Reached start of function" << std::endl;
    ...
    // std::cout << "Reached before return value" << std::endl;
...
pybind11::list privateer::pyanalysis::GlycosylationComposition_memsafe::get_torsions_zscore_summary_with_pdb(OfflineTorsionsZScoreDatabase &importedDatabase, std::string input_pdb_code)
{
...
pybind11::dict privateer::pyanalysis::GlycanStructure::query_glycomics_database(OfflineGlycomicsDatabase &importedDatabase, bool returnClosestMatches, bool returnAllPossiblePermutations, int nThreads)
{
    ...
    else
    {
        ...
        else if (returnClosestMatches && currentGlycan.number_of_nodes() > 1)
        {
            ...
            {
                std::cout << std::endl
                          << "THREADING: Privateer will use " << nThreads << " of the " << detectedThreads << " available threads..." << std::endl;
            }

            std::vector<std::pair<clipper::MGlycan, std::vector<int>>> alternativeGlycans;

            if (useParallelism)
                alternativeGlycans = generate_closest_matches_parallel(currentGlycan, glycomics_database, returnAllPossiblePermutations, false, nThreads, useParallelism);
            else
                alternativeGlycans = generate_closest_matches_singlethreaded(currentGlycan, glycomics_database, returnAllPossiblePermutations, false);
...
pybind11::list privateer::pyanalysis::GlycanStructure::get_torsions_per_linkage_summary()
{
    ...
    for (int i = 0; i < torsion_summary_of_glycan.size(); i++)
    {
        ...
        for (int j = 0; j < current_torsion.linkage_descriptors.size(); j++)
        {
            for (int k = 0; k < current_torsion.torsions.size(); k++)
            {
                ...
                auto current_pair_dict = pybind11::dict("first_residue_name"_a = first_residue_name, "second_residue_name"_a = second_residue_name, "donor_pos"_a = current_torsion.linkage_descriptors[j].first, "acceptor_pos"_a = current_torsion.linkage_descriptors[j].second, "phi"_a = current_torsion.torsions[k].first, "psi"_a = current_torsion.torsions[k].second);
                output.append(current_pair_dict);
            }
        }
    }
            ...
            if (current_linkage.linkage_descriptors.size() == current_linkage.torsions.size())
            {
                for (int linkage_descriptor_index = 0; linkage_descriptor_index < current_linkage.linkage_descriptors.size(); linkage_descriptor_index++)
                {
                    ...
                    if (search_result_in_torsions_zscore_db != std::end(torsions_zscore_database.database_array))
                    {
                        ...
                        {
                            auto current_pair_dict = pybind11::dict("acceptor_sugar"_a = acceptor_sugar, "donor_sugar"_a = donor_sugar, "donor_position"_a = donor_position, "acceptor_position"_a = acceptor_position, "root_descr"_a = root_string, "phi"_a = currentPhi, "psi"_a = currentPsi, "zscore"_a = linkage_score, "donor_atom"_a = donor_atom, "acceptor_atom"_a = acceptor_atom);
                            output.append(current_pair_dict);
                        }
                        else
                        {
                            auto current_pair_dict = pybind11::dict("acceptor_sugar"_a = acceptor_sugar, "donor_sugar"_a = donor_sugar, "donor_position"_a = donor_position, "acceptor_position"_a = acceptor_position, "root_descr"_a = root_string, "phi"_a = currentPhi, "psi"_a = currentPsi, "zscore"_a = pybind11::cast<pybind11::none>(Py_None), "donor_atom"_a = donor_atom, "acceptor_atom"_a = acceptor_atom);
                            output.append(current_pair_dict);
                        }
                    }
                }
...
void privateer::pyanalysis::CarbohydrateStructure::pyinit(clipper::MGlycan &mglycan, const int sugarID, const int glycanID, privateer::pyanalysis::GlycosylationComposition &parentGlycosylationComposition, privateer::pyanalysis::GlycanStructure &parentGlycanStructure)
{
    ...
    auto cremer_pople = inputSugar.cremer_pople_params();

    this->sugar_ring_cremer_pople_q = cremer_pople[0];
    this->sugar_ring_cremer_pople_phi = cremer_pople[1];
    this->sugar_ring_cremer_pople_theta = cremer_pople[2];
...
void privateer::pyanalysis::CarbohydrateStructure::pyinitLigand(const int sugarID, std::vector<std::pair<clipper::String, clipper::MSugar>> &inputSugarList, privateer::pyanalysis::GlycosylationComposition &parentGlycosylationComposition)
{
    ...
    auto cremer_pople = inputSugar.second.cremer_pople_params();
...
{
    auto dict = pybind11::dict("sugarID"_a = sugarID,
                               "glycanID"_a = glycanID,
                               "sugar_pdb_id"_a = sugar_pdb_id,
                               "sugar_pdb_chain"_a = sugar_chain_id,
                               "sugar_seqnum"_a = sugar_seqnum,
                               "sugar_name_full"_a = sugar_name_full,
                               "sugar_name_short"_a = sugar_name_short,
                               "sugar_anomer"_a = sugar_anomer,
                               "is_sane"_a = sugar_sane,
                               "ExperimentalData"_a = updatedWithExperimentalData,
                               "RSCC"_a = sugar_rscc,
                               "mFo"_a = sugar_accum,
                               "BFac"_a = sugar_bfactor,
                               "Q"_a = sugar_ring_cremer_pople_q,
                               "Phi"_a = sugar_ring_cremer_pople_phi,
                               "Theta"_a = sugar_ring_cremer_pople_theta,
                               "OccupancyCheck"_a = sugar_occupancy_check,
...
void privateer::pyanalysis::XRayData::read_from_file(std::string &path_to_mtz_file, std::string &path_to_model_file, std::string &input_column_fobs_user, float ipradius, int nThreads, bool disable_torsions, bool debug_output)
{
    ...
    {
        std::cout << std::endl
                  << "THREADING: Privateer will use " << nThreads << " of the " << std::thread::hardware_concurrency() << " available threads..." << std::endl;
    }
    ...
    std::cout << std::endl
              << " " << fobs.num_obs() << " reflections have been loaded";
    std::cout << std::endl
              << std::endl
              << " Resolution " << hklinfo.resolution().limit() << "Å" << std::endl
              << hklinfo.cell().format() << std::endl;
    ...
    if ((difference > 0.15) || (clipper::Util::is_nan((FobsFcalcSum / FobsSum))))
    {
        ...
        std::cout << std::endl
                  << " The studied portions of the model account for a very significant part of the data. Calculating RSCC against a regular 2mFo-DFc map" << std::endl;
    }
    ...
    if (useParallelism && nThreads >= 2)
    {
        ...
        for (size_t i = 0; i < nThreads; i++)
        {
            ...
            thread_results.push_back(std::async(
                std::launch::async, [](std::vector<clipper::MMonomer> &sugarList, clipper::String &path_to_model_file_clipper, std::vector<std::pair<clipper::String, clipper::MSugar>> &ligandList, clipper::HKL_info &hklinfo, clipper::Grid_sampling &mygrid, clipper::Xmap<float> &sigmaa_all_map, clipper::Xmap<float> &sigmaa_omit_fb, clipper::Xmap<float> &sigmaa_omit_fd, clipper::Xmap<float> &ligandmap, clipper::MGlycology &mgl, bool showGeom, float ipradius, int pos_slash, bool useSigmaa, bool rscc_best, size_t start, size_t end, bool debug_output)
                {
...
void privateer::pyanalysis::CryoEMData::read_from_file(std::string &path_to_mrc_file, std::string &path_to_model_file, float resolution, float ipradius, int nThreads, bool disable_torsions, bool debug_output)
{
    ...
    {
        std::cout << "\n\nUnable to calculate Fo-DFc structure factors for cryoEM map input. Exiting..." << std::endl
                  << std::endl;
    }
    ...
    if (useParallelism && nThreads >= 2)
    {
        ...
        for (size_t i = 0; i < nThreads; i++)
        {
            ...
            thread_results.push_back(std::async(
                std::launch::async, [](std::vector<clipper::MMonomer> &sugarList, clipper::String &input_model, std::vector<std::pair<clipper::String, clipper::MSugar>> &ligandList, clipper::HKL_info &hklinfo, clipper::Grid_sampling &mygrid, clipper::Xmap<double> &cryo_em_map, clipper::Xmap<double> &ligandmap, clipper::MGlycology &mgl, bool showGeom, float ipradius, int pos_slash, size_t start, size_t end, bool debug_output)
                {
                for(size_t index = start; index < end; index++)
                {
                    float x,y,z,maxX,maxY,maxZ,minX,minY,minZ;
...
void init_pyanalysis(py::module &m)
{
    ...
    py::class_<pa::GlycosylationComposition>(m, "GlycosylationComposition")
        ...
        .def("get_torsions_per_linkage_summary", &pa::GlycosylationComposition::get_torsions_per_linkage_summary)
        .def("get_torsions_zscore_summary", &pa::GlycosylationComposition::get_torsions_zscore_summary)
