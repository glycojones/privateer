...
float privateer::util::calculate_linkage_zscore(float phi, float psi, privateer::json::TorsionsZScoreDatabase& matched_linkage, std::string input_code)
{
    ...
    std::vector<std::string> pdb_list = matched_linkage.pdb_list;

    auto pdb_search_iterator = std::find(pdb_list.begin(), pdb_list.end(), input_code);
...
void privateer::glycanbuilderplot::Plot::write_svg_definitions_ostringstream( std::ostringstream& of )
{
    of << "  <defs>\n"
       ...
       << "    <!-- shBond --> "
       << "<line x1=\"-3\" y1=\"0\" x2=\"110\" y2=\"0\" style=\"stroke:" << get_colour(corvette, original_colour_scheme, inverted_background ) << "; stroke-width:15; stroke-linecap:round;\" id=\"shadedbond\" />\n"
...
{
    privateer::glycanbuilderplot::Plot plot(false, original_colour_scheme, "demo", inverted_background );
    plot.plot_demo ( );
