...
if __name__ == "__main__":
    ...
    for count, line in enumerate(jsonObject):
        ...
        if responseGlyConnect.status_code == 200:
            ...
            alternativeAPIGlyConnectID = return_glyconnect_id_from_gtc_external_id(
                glytoucanID
            )
            if glytoucanID in glyconnectHTTP500Exceptions:
                ...
                line[
                    "glyconnect"
                ] = "Unable to match GlyTouCan ID in Glyconnect database as HTTP 500 error was returned, nor it is described in glyconnectHTTP500Exceptions dict. Please report this to hb1115@york.ac.uk"
                numFailedToMatch += 1
        else:
            try:
                ...
                if glytoucanID in glyconnectHTTP500Exceptions:
                    line["glyconnect"] = str(
                        glyconnectHTTP500Exceptions[glytoucanID]["id"]
                    )
                elif alternativeAPIGlyConnectID != "NotFound":
                    line["glyconnect"] = alternativeAPIGlyConnectID
                else:
    ...
    print(
        f'Failed to match {numFailedToMatch} GlyConnect IDs in GlyConnect database out of {len(jsonObject)}. Please CTRL + F for "Unable" in output file and modify the exception list if possible.'
    )
