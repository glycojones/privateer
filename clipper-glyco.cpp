		...
		if ( ring_atoms[1].element().trim() == "C" )        // the atom in position 1 is the anomeric carbon, let's identify it's substituent:
		{
			...
			for ( int i = 0 ; i < neighbourhood.size() ; i++ )
			{
				...
				// the target substituent could be anything apart from H, in-ring oxygen or in-ring carbon
				{
					...
					const int poly = neighbourhood[i].polymer();
					const int mono = neighbourhood[i].monomer();
					const int atom = neighbourhood[i].atom();

					double distance = 0.0;

					if ( neighbourhood[i].symmetry() == 0 )
					{
						distance = clipper::Coord_orth::length(mmol.atom(neighbourhood[i]).coord_orth(), ring_atoms[1].coord_orth());
					}
					else // this neighbour is actually a symmetry mate, so let's find the symmetry operator that places a copy of the molecule close to the anomeric carbon
					{
						...
						clipper::Coord_frac f1 = mmol[poly][mono][atom].coord_orth().coord_frac(mmol.cell());
						clipper::Coord_frac f2 = ring_atoms[1].coord_orth().coord_frac(mmol.cell());
						f1 = spgr.symop(neighbourhood[i].symmetry()) * f1;
						f1 = f1.lattice_copy_near( f2 );
						distance = sqrt(( f2 - f1 ).lengthsq( mmol.cell() ));

						//#ifdef DUMP
						//	DBG << "f1 (symm mate): " << f1.coord_orth(mmol.cell()).format() << "f2 (anomeric carbon): " << f2.coord_orth(mmol.cell()).format() << std::endl;
						//#endif
					}
					...
					if (bonded(mmol.atom(neighbourhood[i]), ring_atoms[1])) //( (distance > 1.20) && (distance < 1.65) ) // was 1.61

						if ( !is_part_of_ring(mmol.atom(neighbourhood[i]), ring_atoms))//  && (ring_atoms[i].occupancy() == mmol.atom(neighbourhood[i]).occupancy() )) // eliminate ring neighbours
						{
							//#ifdef DUMP
							//	DBG << "checking a possible alternate conformer... neighbour is from " << mmol[poly][mono].id().trim() 
							//		<< " and origin is from " << mm.id().trim() << std::endl;
							//#endif

							//if (mmol[poly][mono].id().trim() == mm.id().trim()) // check alternate conformations...
							//{
