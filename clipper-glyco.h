    ...
    */
	...
	class MSugar : public clipper::MMonomer
	{
		...
		public:
			...
			clipper::String conformation_name()
				{ return clipper::data::conformational_landscape[sugar_conformation]; } //!< get a fixed three-character code for the conformation
			clipper::String conformation_name_iupac()
				{ return clipper::data::iupac_conformational_landscape[sugar_conformation]; } //!< get HTML-formatted, iupac-compliant codes describing the conformation of the sugar ring
			clipper::ftype puckering_amplitude()
				{ return sugar_cremer_pople_params[0]; } //!< convenience function for getting the puckering amplitude (in Angstroems) of the sugar ring
			int conformation_code()  
				{ return sugar_conformation; } //!< function for working with a numerical denomination of sugar conformation
			const clipper::MAtom& anomeric_carbon()
				{ return this->sugar_anomeric_carbon; } //!< returns the anomeric carbon
			const clipper::MAtom& anomeric_substituent()
				{ return this->sugar_anomeric_substituent; } //!< returns the anomeric substituent
			const clipper::MAtom& configurational_carbon()
				{ return this->sugar_configurational_carbon; } //!< returns the configurational carbon
			const clipper::MAtom& configurational_substituent()
				{ return this->sugar_configurational_substituent; } //!< returns the configurational substituent
			clipper::String anomer()
				{ return sugar_anomer; } //!< convenience function to get the anomer type, e.g. "alpha"
			clipper::String handedness()
				{ return sugar_handedness; } //!< get the sugar's handedness: "D" for dextro, "L" for laevo, "N" for undetermined and "X" for unsupported (missing from the clipper::data sugar database)
			clipper::String type_of_sugar()
				{ return sugar_denomination; } //!< returns a string containing an anomer-handedness-type denomination, e.g. "beta-D-aldopyranose"
			int ring_cardinality()
				{ return sugar_ring_elements.size(); } //!< get the number of atoms forming the main sugar ring
			std::vector<clipper::MAtom> ring_members()
				{ return sugar_ring_elements; } //!< returns an standard vector containing those MAtom's forming the ring
			const clipper::Coord_orth& ring_centre()
				{ return this->sugar_centre; } //!< get the ring's centre (original coordinates)
			const clipper::Vec3<ftype>& ring_mean_plane()
				{ return sugar_mean_plane; } //!< get the vector normal to the sugar ring's mean plane, with origin in ring_centre()
			std::vector<ftype> cremer_pople_params()
				{ return sugar_cremer_pople_params; } //!< returns Cremer-Pople parameters (Cremer and Pople, 1975) in the form { Q , Angle1, ... }
			int potential_linkages()
				{ return sugar_linked_to.size(); } //!<  returns the number of potential linkages to other sugars
			bool is_sane() //!< checks it against the internal clipper::data sugar database for correct anomer, handedness and ring members
				{ return sugar_sane; } 
            clipper::String full_name() 
				{ return sugar_name; }
			clipper::String full_type()
				{ return sugar_type; }
			std::vector<clipper::ftype> ring_angles()
				{ return sugar_ring_angles; }
			std::vector<clipper::ftype> ring_bonds()
				{ return sugar_ring_bonds; }
			std::vector<clipper::ftype> ring_torsions()
				{ return sugar_ring_torsion; }
			clipper::ftype ring_bond_rmsd()
	...
	class MGlycan
	{
		...
		public:
			...
			MGlycan(const clipper::MiniMol&);
			const std::vector<clipper::MSugar> detected_sugar_list() const;
			const std::vector<clipper::MSugar> n_glycan_roots() const;
			const std::vector<clipper::MSugar> o_glycan_roots() const;

		private:

			std::vector<clipper::MSugar> detected_sugars;
