...
int main(int argc, char** argv)
{
  ...
  while ( ++arg < args.size() ) {
	...
	{
      if ( ++arg < args.size() ) ipcif = args[arg];
	} 
	...
	{
      if ( ++arg < args.size() ) if (clipper::String(args[arg]) == "batch") batch = true;
    }
	else if ( args[arg] == "-radiusin" ) 
	{
      	if ( ++arg < args.size() ) 
		{
			ipradius = clipper::String(args[arg]).f(); if (ipradius < 1.0) { std::cout << "\n\nMask radius is too small!" << std::endl << std::endl; prog.set_termination_message( "Failed" ); return 1;  } 
		}
    }
  ...
  if ( (ippdb == "NONE") || ((ipcif == "NONE") && (ipmtz == "NONE") && (noMaps == false)) )
  {
		...
		std::cout << "\t-cifin <.cif> OR -mtzin <.mtz>\tCOMPULSORY: a CIF file containing amplitudes or an MTZ file containing intensities and/or amplitudes" << std::endl;
		std::cout << "\t\t\t\t\tIf intensities are supplied, Privateer will run the CCP4 program 'ctruncate' in order to produce amplitudes. \n\t\t\t\t\tPlease ensure that CCP4 software is within reach (i.e. the CBIN variable points to it)" << std::endl;
		std::cout << "\t-mtzout <.mtz>\t\t\tOutput best and difference map coefficients to the specified MTZ file" << std::endl; 
		std::cout << "\t-colin-fo\t\t\tColumns containing F & SIGF, e.g. FOBS,SIGFOBS, without any spaces in between characters. \n\t\t\t\t\tIf not supplied, Privateer will try to guess the path using the most commonly used choices" << std::endl;
	...
	if (!batch) std::cout << std::endl << "Reading " << ippdb.trim().c_str() << "... "; fflush(0);
    ...
    { 
		...
		if (!batch) std::cout << "Reading " << ipmtz.trim().c_str() << "... "; fflush(0);
    ...
    { 	// assume CIF file format instead
		if (!batch) std::cout << "Opening CIF file: " << ipcif.trim() << std::endl;
    ...
    if (useMTZ)
    {
        ...
        if (notFound)
        {
            ...
            else sprintf(cmd, "$CBIN/ctruncate -hklin %s -mtzout amplitudes.mtz -colin '/*/*/[I,SIGI]' >/dev/null", ipmtz.c_str());
            ...
            {
                if (!batch) std::cout << "\nReading output from ctruncate...\n" << "Previous hklinfo: " << hklinfo.cell().format() << " " << hklinfo.spacegroup().spacegroup_number() << " " << hklinfo.num_reflections() << "\n";
    ...
    if (!batch) std::cout << std::endl << " " << fobs.num_obs() << " reflections have been loaded" << std::endl << std::endl << " Resolution " << hklinfo.resolution().limit() << "Å" << std::endl << hklinfo.cell().format() << std::endl;
	...
	if (!batch) std::cout << "done." << std::endl << "Computing 2mFo-DFc, mFo-DFc and mFo-omit_DFc maps... "; fflush(0);
...
#pragma omp parallel sections
{
	...
	if ( output_mtz )
	{
		if (!batch) std::cout << "Writing map coefficients to " << opfile << "... "; fflush(0);
		if (useMTZ) 
		{
	...
	{
	 	useSigmaa = true;
		if (!batch) std::cout << std::endl << " The studied portions of the model account for a very significant part of the data. Calculating RSCC against a regular 2mFo-DFc map" << std::endl; 
    }

	if (!batch) std::cout << "\nWriting maps to disk... "; fflush(0);
	...
	if (useSigmaa) ms = clipper::Map_stats(sigmaa_all_map); 
	else ms = clipper::Map_stats(sigmaa_omit_fd);
...
#pragma omp parallel sections
{
    ...
    for (int index = 0; index < ligandList.size(); index++)
    {
		...
		if (useSigmaa) i0 = clipper::Xmap_base::Map_reference_coord( sigmaa_all_map, origin.coord_frac(hklinfo.cell()).coord_grid(mygrid) );
        else i0 = clipper::Xmap_base::Map_reference_coord( sigmaa_omit_fd, origin.coord_frac(hklinfo.cell()).coord_grid(mygrid) );
            ...
            for ( iv = iu; iv.coord().v() <= destination.coord_frac(hklinfo.cell()).coord_grid(mygrid).v(); iv.next_v() )
                for ( iw = iv; iw.coord().w() <= destination.coord_frac(hklinfo.cell()).coord_grid(mygrid).w(); iw.next_w() )
                {
                	if ( mask[iw] == 1.0)
					{
						...
						if (useSigmaa) meanDensityExp = meanDensityExp + sigmaa_all_map[iw];
						else meanDensityExp = meanDensityExp + sigmaa_omit_fd[iw];
